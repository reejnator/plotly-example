...
// This library is derived from Plotl's Arduino example.
// It is modified to work with the Spark Core.
// Spark Core modifications are indicated with: // =====>>>>
...
// Maybe Spark Core is not able to resolve plot.ly ?
    while ( !client.connect("www.plot.ly", 80) ) {
        if(log_level < 4){
...
// The original example had a while loop that checked the client was connected. 
// With the Spark Core (and myabe others) the core stayed in this loop until the client considered itself disconnected.
// That never happened: the core stayed in this loop forever, altough no data was available. 
// Modified this to stay in the loop as long as connected AND data available.
// The extra !proceed condition is to make sure that the loop can only exit once the "All Streams Go!" string has been received
// (to make sure that the loop is not exiting immediately, even before data has been received)
// This seems to work
...
// =====>>>>
// next line was originally uncommented:
//        client.stop();
    }
